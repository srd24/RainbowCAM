...
\tom{I have added two comments here about the rest of the complex $\lambda$ plane. Feel free to edit.}

We find no evidence of Regge poles in the fourth quadrant when $\omega>0$. In addition, we find that $\lambda_n(-\omega) = \lambda_n(\omega)^*$ so that there are Regge poles in the fourth quadrant when $\omega<0$. Here we will only consider $\omega>0$. 

We propose that the good analytical extension of modes into the second and third quadrants is to enforce the regular boundary condition for Re$[\lambda]<0$, i.e. $\phi_{\omega\ell} \sim r^{-\lambda+1/2}$ as $r\rightarrow 0$. Since the wave equation is invariant under the transformation $\lambda \rightarrow -\lambda$, it can be seen that the distribution of poles in the left half complex plane - Re$[\lambda]<0$ - follows from the poles in the right half complex plane by reflection through the origin. 
