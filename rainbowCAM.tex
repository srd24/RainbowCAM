...
As far as we know, the only published study into Regge pole spectra for relativistic stellar models was that of Chandrasekhar and Ferrari in 1991 \cite{ChandrasekharIV:1992ey}. %They used Regge pole theory to calculate the flow of gravitational energy through a star. They restricted their attention to resonant modes, i.e. poles with small imaginary parts. For the axial sector these are only present for UCOs. For the polar sector the situation is more complicated, as there are long lived fluid pulsation modes.
They considered resonant modes (poles with small imaginary parts corresponding to trapped $w$-modes or fluid modes), and used CAM theory to calculate the flow of gravitational energy through a star. More literature exists on Regge pole theory applied to black hole scattering, for example Andersson and Thylwe have described scattering of scalar waves by a Schwarzschild BH utilising CAM techniques \cite{Andersson:1994rk}. They associate the BH Regge poles with ``surface waves'' localised near the unstable light ring at $r=3M$. Later, Decanini, Folacci and Jensen showed that the Schwarzschild BH QNMs are in fact Breit-Wigner type resonances generated by said surface waves \cite{Decanini:2002ha}. We see here that CAM theory is a powerful approach to BH physics, that provides a clear and robust physical picture. It seems reasonable to expect that CAM may also aid in our understanding of other compact objects such as UCOs and neutron stars. With this in mind, we now turn our attention to calculating the Regge pole spectrum for compact bodies.
