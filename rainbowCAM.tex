...
\end{align}
\end{subequations}
The constant-density model can be thought of as representing the $n=0$ limit of the family of polytropes.
...
V^{(s)}_{\ell}(r) =\left(1-\frac{2M}{r}\right)\left(\frac{\ell(\ell+1)}{r^2}+\frac{2M(1-s^2)}{r^3}\right)
\end{equation}
...
\Delta V_{\ell}^{(ax)} &= - \frac{3Mf(R)}{R^3} ,
\end{align}
\end{subequations}
where
\begin{equation}
...
\caption{\label{RP_approx_2Mw_3_6_s_1} The effective potential $V_\ell$ for a quadrupole ($\ell =2$) perturbation of a compact body of constant density and tenuity $R/M = 6$ (upper) and $R /M = 2.26$ (lower). The scalar field and axial gravitational-wave potentials are indicated as solid/black lines and dotted/red lines, respectively. The horizontal axis is the tortoise coordinate $r_\ast/(2M)$ defined in Eq.~(\ref{eq:tortoise}).}
\label{fig:Veff}
\end{figure}
...
\subsection{Boundary conditions and scattering}
The modes $\phi_{\omega \ell}$ should have a regular behaviour at the centre of the object ($r = 0$), and inspection of the radial equation (\ref{H_Radial_equation}) shows that
...
\section{The Regge pole spectrum}
 ...
 \subsection{Regge poles and quasinormal modes}
...
In order to determine the Regge poles we use the method described in sec.~4 of ref.~\cite{Benhar:1998au}. In fact, they had adapted the \textit{continued fraction method}, developed by Leaver in the case of BH-QNM frequencies~\cite{Leaver:1985ax}, to determine the axial QNM frequencies of a compact bodies. Though they used their method to find the QNM frequencies, their method is equally valid, \textit{mutatis mutandis}, for finding the Regge poles. We have implemented numerically this method by using the Hill determinant approach of Majumdar and Panchapakesan~\cite{mp}, or more precisely, by modifying their method and adapting it to our problem, \textit{i.e.}, using it for a 4-term recurrence relation.
...
\subsection{Classification of quasinormal modes}
