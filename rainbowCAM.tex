...
The $w$-modes (quasinormal modes) may be divided into three branches:
	\begin{enumerate}
   \item Curvature modes, the standard $w$-modes extant for all relativistic stars. The less compact the star the more rapid the damping (Im$\{\omega\}$ increases with $R/M$). \sam{Anything else to say on curvature modes?} \tom{have added a little}
...
For more details on quasinormal modes see \sam{Appropriate reference needed.} \tom{Can we just get rid of this sentence as have already referred to review by Kokkotas?}
...
\tom{Some of this paragraph may be more suited to the introduction?}\\
\moh{Reference Added for Decanini, Folacci and Jensen}.
...
the case of Reissner-Nordstöm black holes \cite{Leaver:1990zz}, \textit{i.e.}, reducing the four-term recurrence relations \eqref{Recurrence_4_terms} to a three-term recurrence relation by using the gaussian elimination step.
...
\\
\moh{initial conditions added for $D_n$}
