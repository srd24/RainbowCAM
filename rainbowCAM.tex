...
and its complex conjugate
 \begin{equation}\label{Wave_equation_reg_conjugate}
   \frac{d^2}{dr_*^2}\overline{\phi}_{\omega,\lambda-1/2}(r)+ \left(\omega^2 -V_{\overline{\lambda}-1/2}\right)\overline{\phi}_{\omega,{\lambda-1/2}}(r) =0
 \end{equation}
Multiplying \eqref{Wave_equation_reg} by $\overline{\phi}_{\omega,\lambda-1/2}(r)$ and \eqref{Wave_equation_reg_conjugate} by $\phi_{\omega,\lambda-1/2}(r)$ and subtracting, it follows,
\begin{widetext}
\begin{equation}\label{Derivative_Wronskian}
\begin{aligned}
 \overline{\phi}_{\omega,\lambda-1/2}(r) \frac{d^2}{dr_*^2}\phi_{\omega,\lambda-1/2}(r) - \phi_{\omega,\lambda-1/2}(r) \frac{d^2}{dr_*^2} \overline{\phi}_{\omega,\lambda-1/2}(r) &= 2 i|\phi_{\lambda-1/2}(\omega)|^2 f[r]\frac{2\text{Re}[\lambda]\text{Im}[\lambda]}{r^2} \\
                           &= 2 i|\phi_{\lambda-1/2}(\omega)|^2 f[r]\frac{\text{Im}[\lambda^2]}{r^2} 
 \end{aligned}
\end{equation}
\end{widetext}
In Eq.~\eqref{Derivative_Wronskian}, the l.h.s is the derivative of the Wronskian, $W$, of the two solutions respect to $r_*$ and it is a constant (\textit{i.e.}, $d/dr_* W[\overline{\phi},\phi] = 0$) if $\lambda$ either real or purely imaginary. According to Eq.~\eqref{Derivative_Wronskian}, we can write
\begin{equation}\label{Derivative_Wronskian_bis}
\frac{d}{dr_*}W[\overline{\phi},\phi]= 2i|\phi_{\lambda-1/2}(\omega)|^2 f[r]\frac{2\text{Re}[\lambda]\text{Im}[\lambda]}{r^2}
\end{equation}

Integration of \eqref{Derivative_Wronskian_bis} gives
\begin{equation}\label{Evaluation_W}
\lim_{r_*\rightarrow \infty}  W[\overline{\phi},\phi] = 4i\, \text{Im}[\lambda]\, \text{Re}[\lambda] \int_{0}^{+\infty} \frac{|\phi_{\lambda-1/2}(\omega)|^2}{r^2}\,dr
\end{equation}
...
\lim_{r_*\rightarrow \infty}  W[\overline{\phi},\phi] =-2i\, \omega\left(|A^{(-)}_{\lambda-1/2}|^2-|A^{(+)}_{\lambda-1/2}|^2\right)
\end{equation}
...
It is important to note that we have, under transformation $\omega \rightarrow -\omega$, the coefficient $A^{(-)}_{\lambda-1/2}(\omega)$ defined by the problem  \eqref{H_Radial_equation}--\eqref{bc_1_in}--\eqref{bc_2_in}, satisfy
\begin{equation}\label{symmetry_relation}
 A^{(-)}_{\lambda-1/2}(-\omega) = \left[A^{(-)}_{\lambda^*-1/2}(\omega)\right]^*
\end{equation}
hence, due to this symmetry relation, we get 
...
The results established here agree with the situation in non-relativistic wave and atom/particle scattering (see \textit{e.g.} \cite{Newton:1982qc,Bottino1962} and references therein), also with the Schwarzschild black hole scattering \cite{Andersson:1994rk}. 
