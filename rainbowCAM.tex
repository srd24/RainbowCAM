...
Time-independent scattering by a compact body of radius $R$ with a regular centre, such as a neutron star or white dwarf, has received comparatively less attention. In such a scenario, an electromagnetic wave will not penetrate far inside the compact body; but on the other hand, a gravitational wave will pass through the body without much impediment from the matter distribution. Similarly, a neutron star is expected to be substantially transparent to neutrinos. Even in cases where the coupling to matter is negligible, the incident wave will nevertheless be scattered by the influence of the spacetime curvature. Thus, the resulting scattering pattern depends on $M\omega$, on the internal structure of the body, and on its inverse compactness or \emph{tenuity} $R/M$. Typical values of the tenuity are: $R/M \sim 6$ for neutron stars, $\sim 1.4-9.4 \times 10^3$ for white dwarfs, $4.7 \times 10^5$ for the Sun, and $1.4 \times 10^9$ for Earth. More speculatively, we shall also consider the case of (hypothetical) Ultra Compact Objects (UCO), whose tenuity is bounded from below by the Buchdahl bound of $R / M = 9/4$. 

Studies of black hole scattering typically involve the calculation of scattering amplitude(s) via partial-wave expansions.  A powerful approach for resumming these partial-wave expansions is provided by so-called Complex Angular Momentum (CAM) techniques. In the CAM approach, the sum over partial waves is replaced by an integral, and the contour of integration is deformed into  the complex-angular-momentum plane such that one collects a sum over the residues of simple poles: these are the so-called \emph{Regge poles}. 

CAM theory, originally developed for high-energy physics applications, has been successfully applied to black hole scattering scenarios. In 1994, Andersson and Thylwe \cite{Andersson:1994rk} described the scattering of scalar waves by a Schwarzschild BH by utilising CAM techniques, and they showed that, in this case, Regge poles are associated with ``surface waves'' localised near the unstable light ring at $r=3M$ (later it was shown that the corresponding quasinormal modes are Breit-Wigner type resonances generated by these surface waves \cite{Decanini:2002ha}). Recently, Folacci and Ould Elhadj have shown that CAM methods can be used for precise numerical calculations of scattering cross sections of scalar fields, electromagnetic fields \cite{Folacci:2019cmc} and gravitational waves  \cite{Folacci:2019vtt} on the Schwarzschild spacetime. Moreover, they have obtained an approximate  formula (see Sec.~IV in Ref.~\cite{Folacci:2019vtt}) that accurately describes both the `orbiting' and `glory' oscillations in black hole scattering cross sections in the short-wavelength regime. This approximation arises from applying WKB theory to characterise the Regge pole with the smallest imaginary part, and its residue. \sam{Mohamed, please correct this if wrong.} 

CAM techniques have not yet been applied to compact-body scattering, it appears, with one notable exception: in 1991, Chandrasekhar and Ferrari \cite{ChandrasekharIV:1992ey} examined the Regge pole spectra for relativistic stellar models. 
Their study focussed on resonant modes: poles with small imaginary parts corresponding to trapped $w$-modes or fluid modes, and it employed Complex Angular Momentum (CAM) theory to calculate the flow of gravitational energy through a star.


 It seems reasonable to expect that CAM may also aid in our understanding of other compact objects such as UCOs and neutron stars. With this in mind, we now turn our attention to calculating the Regge pole spectrum for compact bodies.

It was recently shown \cite{Dolan:2017rtj, Stratton:2019deq} that time-independent scattering by compact bodies generates (in principle) the \emph{rainbow scattering} phenomenon in the short wavelength regime ($M \omega \gg 1$). The rainbow scattering phenomenon is linked to the formation of a cusp caustic in the incident wavefront (see Fig.~1 in Ref.~\cite{Stratton:2019deq}), and to the stationary point in the geodesic deflection function associated with a ray (that is, a null geodesic) that passes somewhat inside the body. For dilute bodies, such as stars and white dwarfs, the cusp caustic forms at a distance $d \sim R^2 / (4M)$ downstream of the body, and the rainbow angle is $\theta_r \approx 4M/R$ (e.g.~for the Sun, $d \approx 550 \, \text{a.u.}$ and $\theta_r \approx 1.8 \, \text{arcsec}$). For compact bodies such as neutron stars, however, the cusp caustic forms near the surface of the body, and the rainbow angle is sizable ($\theta_r \gtrsim 59.6^\circ$ for $R/M = 6$; see Table I in Ref.~\cite{Stratton:2019deq}). In this case, the rainbow angle is -- in principle at least -- a diagnostic of the matter distribution within the body and thus its nuclear equation of state. 
...
Other recent works on the theme of scattering by a compact body include studies of: scattering by compact objects with an absorbing surface \cite{Nambu:2019sqn}; time-domain simulations with finite-element-method methods \cite{He:2019orl}; gravitationally-induced interference patterns in flavour oscillations in neutrino astronomy \cite{Alexandre:2018crg}; and the use of continuous sources of gravitational waves to probe stellar structure \cite{Marchant:2019swq}.
 ...
 \cite{Folacci:2018sef} \cite{Decanini:2011xi, Decanini:2011xw}

to study time-independent scattering by a compact body. 

More literature exists on Regge pole theory applied to black hole scattering, for example They . Later,  We see here that CAM theory is a powerful approach to BH physics, that provides a clear and robust physical picture. 
...
%In the polytropic cases $n > 0$, the functions are more regular \sam{can we make a precise statement?}\tom{I will write something here} at the surface, but not $C^\infty$ (i.e.~not smooth). As we shall see, the breakdown of smoothness leads to consequences for the Regge pole spectrum.
For a general polytrope with $n>0$ and $m$ the smallest integer such that $m \geq n$, $h(r)$ is $C^m$ and $f(r)$ is $C^{m+1}$ at the surface.
%\tom{This can be seen from equations relating $(m,\rho,p,f,h)$ and by noting $\rho \sim a(R-r)^n$ at the surface \cite{Lindblom:1983ps} but we don't mention these.}
...
%
\moh{Remark for Tom and Sam : In the next paragraph, I modified it to include the Regge poles of the third quadrant by symmetry to the origin $O$ , in the others words by changing $\ell \rightarrow -\ell-1$ } \\
...
\centering
 \includegraphics[scale=0.50]{Scattering_Cross_Section_R_2-dot-26_2Mw_3}
  \vspace*{-0.3cm}
\caption{\label{S_0_R_2-dot-26_2Mw_3_Exact_vs_CAM} The scalar cross section of a very compact bodies for $2M\omega=3$ and $R=2.26M$ and its Regge pole approximation.  The plots show the effect of including successively more Regge poles (plots 1--3). In the final plot, The sum over Regge poles gives a cross section which agrees well with the (regularized) partial-wave sum for intermediate and large values of the scattering
angle.}
\end{figure*}
...
\centering
 \includegraphics[scale=0.50]{Scattering_Cross_Section_R_2-dot-26_2Mw_6}
  \vspace*{-0.3cm}
\caption{\label{S_0_R_2-dot-26_2Mw_6_Exact_vs_CAM} The scalar cross section of a very compact bodies for $2M\omega=6$ and $R=2.26M$ and its Regge pole approximation. The plots show the effect of including successively more Regge poles (plots 1--3). In the final plot, The sum over Regge poles gives a cross section which agrees well with the (regularized) partial-wave sum for intermediate and large values of the scattering
angle.}
\end{figure*}
...
\acknowledgments
M. O. E. H. wishes to thank Antoine Folacci for various discussions concerning this work and thanks Sam R. Dolan for his kind
