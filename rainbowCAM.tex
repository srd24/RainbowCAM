...
\end{equation}
encapsulates the ratio of the gravitational radius $r_g = 2GM/c^2$ to the wavelength $\lambda$ (henceforth we adopt geometric units such that $G=c=1$). The long wavelength ($M \omega \ll 1$), short wavelength ($M \omega \gg 1$) and intermediate regimes have been studied with a combination of perturbative \cite{DeLogi:1977dp,Dolan:2007ut,Guadagnini:2008ha,Sorge:2015yoa}, semi-classical \cite{Matzner:1985rjn, Anninos:1992ih} and numerical methods. The $s = 0$ (scalar) \cite{Matzner:1968,Sanchez:1977vz,Andersson:1995vi,Glampedakis:2001cx,Leite:2019eis}, $s=1/2$ (fermion) \cite{Dolan:2006vj,Cotaescu:2014jca}, $s=1$ (electromagnetic) \cite{Fabbri:1975, Crispino:2009xt, Crispino:2015gua} and $s=2$ (gravitational) cases \cite{MatznerRyan1978,Handler:1980un,Dolan:2008kf} have all been covered.

Time-independent scattering by a compact body with a regular centre, such as a neutron star or white dwarf, has received comparatively less attention. In such a scenario, an electromagnetic wave will not penetrate inside the compact body; but on the other hand, a gravitational wave will pass through the body without impediment from the matter distribution. Similarly, a neutron star is expected to be substantially transparent to neutrinos. Though the coupling to matter is weak, the incident wave is still strongly scattered due to the influence on the spacetime curvature. Recent work \cite{Dolan:2017rtj, Stratton:2019deq} has explored the \emph{rainbow scattering} phenomenon that arises at short wavelengths, due to a stationary point in the geodesic deflection function associated with a ray that passes somewhat inside the body. In principle, the rainbow angle is a diagnostic of the matter distribution of the body, and thus its nuclear equation of state. \cite{Nambu:2019sqn}
...
A widely-studied model is that of a polytropic star, with an EoS $p(\rho) = \kappa \rho^{1+1/n}$, where $n$ is the polytropic index (see e.g.~\cite{Stratton:2019deq}). Here we shall consider a special case: an incompressible perfect fluid ball of uniform density described by Schwarzschild's interior solution for an incompressible fluid \cite{Shapiro1983}, with
\begin{subequations}
\begin{align}
...
\end{align}
\end{subequations}
The constant-density model can be thought of as representing the $n \rightarrow 0$ limit of the family of polytropes.
...
where
\begin{equation}
\Delta V_{\ell} \equiv \lim_{\epsilon \rightarrow 0} \left\{ V_{\ell}(R + \epsilon) - V_{\ell}(R - \epsilon) \right\} .
\end{equation}
...
\section{The Regge pole spectrum}
 ...
 Mathematically, there is a close relationship between quasinormal modes and Regge poles; both are sets of poles of the scattering matrix. Physically, quasinormal modes are most relevant in time-dependent scattering scenarios, and Regge poles in time-independent scattering scenarios.
 
The \emph{quasinormal mode spectrum} is the set of frequencies $\{ \omega_{\ell \hat{n}} \}$ in the complex-$\omega$ plane at which the scattering matrix $ S_{\ell}(\omega)$ has a simple pole, for a real value of $\ell$; that is, $\ell \in \mathbb{R}$ and $\omega_{\ell \hat{n}} \in \mathbb{C}$. 

The \emph{Regge pole spectrum} is the set of angular momenta $\lambda_{\omega \hat{n}} = \ell_{\hat{n}}(\omega) + 1/2$ in the complex-$\lambda$ plane at which the scattering matrix has a simple pole for real $\omega$; that is, $\omega \in \mathbb{R}$ and $\lambda_{\hat{n}}(\omega) \in \mathbb{C}$. Here $\hat{n}$ is an index for enumerating the poles (see below). In all cases considered in this work, the simple poles arise as simple zeros of $A_{\ell}^{(-)}(\omega)$. 

The quasinormal mode spectrum of spherically-symmetric compact objects has been studied in some detail in \cite{Kokkotas:1986gd,Kokkotas:1992ka,Andersson:1997eq,Kokkotas:1999bd,Leins:1993zz}. 
Newly-formed neutron stars, the remnants of supernovae collapse, are predicted to pulsate with a large initial energy \cite{thorne1967non}, and fluid pulsations will surely generate gravitational waves. In 1967, Thorne and Campolattaro \cite{thorne1967non} classified the fluid modes of a relativistic body by analogy with fluid modes in a Newtonian body, with the addition of a damping time due to the emission of GWs. Two decades later, Kokkotas and Schutz showed that an additional family of modes existed \cite{Kokkotas:1986gd,Kokkotas:1992ka}, dubbed $w$-modes. These modes are characterised by a negligible excitation of fluid motion, and in the axial sector, by no fluid motion. They are highly damped and correspond to excitations of the dynamical perturbed space-time. For a review of (gravitational) quasinormal modes in relativistic stars and black holes see \cite{Kokkotas:1999bd}.

The $w$-modes (quasinormal modes) may be divided into three branches:
	\begin{enumerate}
   \item Curvature modes, oscillations whose damping is larger for less compact bodies.
   \item Interface modes ($\omega_{\text{II}}$-modes \cite{Leins:1993zz}), characterised by very rapid damping (i.e.~large negative imaginary part of $\omega_{\ell \hat{n}}$). This branch of modes is most similar to the Schwarzschild black hole quasinormal modes. 
   \item Trapped modes \cite{Chandrasekhar449}: These modes exist for UCOs with $R/M < 3$, as the effective radial potential has a local minimum inside the star, and local maximum near the photon sphere $r=3M$. %The trapped modes are essentially the first few curvature modes with energy less than the potential barrier. 
...
In order to determine the Regge poles we adapt the method of Benhar, Berti and Ferrari (BBF), described in Sec.~4 of Ref.~\cite{Benhar:1998au}, who used it to determine the axial QNM frequencies of compact bodies. In turn, the BBF method is an adaptation of the \textit{continued fraction method} developed by Leaver in the case of black hole-quasinormal mode frequencies~\cite{Leaver:1985ax}. Though BBF used their method to find the QNM frequencies, their method is equally valid, \textit{mutatis mutandis}, for finding the Regge poles. We have implemented numerically this method by using the Hill determinant approach of Majumdar and Panchapakesan~\cite{mp}, or more precisely, by modifying their method and adapting it to our problem, \textit{i.e.}, using it for a 4-term recurrence relation.
...
In this section we present numerical results for the Regge pole spectrum of a compact body, in two cases: (i) a neutron-star-like body with tenuity $R / M = 6$, and (ii) a UCO with a tenuity just above the Buchdahl bound, $R / M = 2.26$. Results for the frequencies $M \omega = 3/2$ and $M \omega = 8$ are compared, for the scalar-field ($s=0$) and axial GW ($s=2$) cases.
...
\begin{threeparttable}[htp]
%\captionsetup{font=small}
\caption{\label{tab:table3} The lowest Regge poles $\lambda_{n}(\omega)$ for the axial gravitational waves. The radius of the compact bodies is $R = 6M$ and we assume $2M=1$.}
\smallskip
\centering
\begin{ruledtabular}
\begin{tabular}{cccc}
 $n$ & $\omega$  & $\lambda^{\text{(S-W)\tnote{1}}}_n(\omega)$ & $\lambda^{\text{(B-R)\tnote{2}}}_n(\omega)$
 \\ \hline
...
\begin{threeparttable}[htp]
%\captionsetup{font=small}
\caption{\label{tab:table4} The lowest Regge poles $\lambda_{n}(\omega)$ for the axial gravitational waves. The radius of the compact bodies is $R = 2.26M$ and we assume $2M=1$.}
\smallskip
\centering
\begin{ruledtabular}
\begin{tabular}{ccccc}
 $n$ & $\omega$  & $\lambda^{\text{(S-W)\tnote{1}}}_n(\omega)$  & $\lambda^{\text{(B-R)\tnote{2}}}_n(\omega)$ & $\lambda^{\text{(N-R)\tnote{3}}}_n(\omega)$
 \\ \hline
